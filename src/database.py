"""
database.py — DB access, sessions, and HTML fragment builders for ООО «Обувь».
All paths are resolved relative to BASE_DIR (project root), never hardcoded.
"""
...
def fetch_categories() -> list[str]:
    conn = get_db()
    rows = conn.execute(
        "SELECT category_name FROM category ORDER BY category_name").fetchall()
    conn.close()
...
def fetch_manufacturers() -> list[str]:
    conn = get_db()
    rows = conn.execute(
        "SELECT manufacturer_name FROM manufacturer ORDER BY manufacturer_name").fetchall()
    conn.close()
...
def save_product(data: dict) -> None:
    ...
    try:
        for tbl, col, val in [
            ...
            ('manufacturer', 'manufacturer_name', data.get('manufacturer_name', '')),
        ]:
            if val:
        ...
        def fk(tbl, col, val):
            if not val:
                return None
            r = conn.execute(f"SELECT {tbl}_id FROM {tbl} WHERE {col}=?", (val,)).fetchone()
        ...
        args = (
            data['article'], data['product_name'], data['unit'],
            float(data['price']), float(data.get('discount', 0)),
...
def fetch_order_by_id(oid: int) -> dict | None:
    ...
    users = conn.execute(
        "SELECT user_id, full_name FROM user ORDER BY full_name").fetchall()
    pps = conn.execute(
        "SELECT pickup_point_id, address FROM pickup_point ORDER BY address").fetchall()
    conn.close()
    return {
        'order':         dict(o),
    ...
    try:
        ...
        if oid:
            conn.execute("""UPDATE "order" SET
                ...
                WHERE order_id=?""",
                (data.get('order_date') or None, data.get('delivery_date') or None,
                 ppid, uid, data.get('pickup_code') or None,
                 data.get('status', 'Новый'), int(oid)))
            conn.execute("DELETE FROM order_item WHERE order_id=?", (int(oid),))
        else:
            mx = conn.execute(
                'SELECT COALESCE(MAX(order_number), 0) AS m FROM "order"'
            ).fetchone()['m']
            conn.execute("""INSERT INTO "order"
        ...
        for item in data.get('items', []):
            ...
            if not art:
                continue
            pr = conn.execute(
                "SELECT product_id FROM product WHERE article=?", (art,)).fetchone()
            conn.execute(
...
def sel_opts(items: list[str], selected: str = '', empty_label: str | None = None) -> str:
...
def product_row_html(p, can_edit: bool = False) -> str:
    ...
    # Photo
    photo   = p['photo']
    img_src = (f'/static/images/{photo}'
               if photo and photo not in ('None', '')
               else '/static/images/picture.png')

    # Price display
    if discount > 0:
        price_html = (f'<span class="price-old">{price:,.2f}\u00a0₽</span>'
                      f'<span class="price-new">{final_p:,.2f}\u00a0₽</span>')
    else:
        price_html = f'<span class="price-plain">{price:,.2f}\u00a0₽</span>'

    # Discount badge
    disc_cell = (f'<span class="disc-badge">{discount:.0f}%</span>'
                 if discount > 0 else '<span class="no-disc">—</span>')
