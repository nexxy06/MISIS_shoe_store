...
def import_pickup_points(conn: sqlite3.Connection) -> None:
    df = pd.read_excel(
        _import_path("Пункты_выдачи_import.xlsx"), dtype=str, header=None
    )
    for _, row in df.iterrows():
...
def import_products(conn: sqlite3.Connection) -> None:
    ...
    for _, row in df.iterrows():
        ...
        cat_id = conn.execute(
            "SELECT category_id     FROM category     WHERE category_name=?", (cat,)
        ).fetchone()["category_id"]
        sup_id = conn.execute(
            "SELECT supplier_id     FROM supplier     WHERE supplier_name=?", (sup,)
        ).fetchone()["supplier_id"]
        man_id = conn.execute(
            "SELECT manufacturer_id FROM manufacturer WHERE manufacturer_name=?", (man,)
        ).fetchone()["manufacturer_id"]
...
def import_orders(conn: sqlite3.Connection) -> None:
    ...
    pp_rows = conn.execute(
        "SELECT pickup_point_id FROM pickup_point ORDER BY pickup_point_id"
    ).fetchall()
    ...
    for _, row in df.iterrows():
        ...
        u = conn.execute(
            "SELECT user_id FROM user WHERE full_name=?", (fio,)
        ).fetchone()
        ...
        ord_row = conn.execute(
            'SELECT order_id FROM "order" WHERE order_number=?', (order_num,)
        ).fetchone()
        if not ord_row:
            continue
        ...
        while i < len(parts):
            ...
            pr = conn.execute(
                "SELECT product_id FROM product WHERE article=?", (article,)
            ).fetchone()
            conn.execute(
