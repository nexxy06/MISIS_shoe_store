"""
server.py — HTTP server for ООО «Обувь».

Folder layout (relative to project root):
    src/          — Python source files (this file lives here)
    static/       — CSS, JS, favicon, logo, product images
    templates/    — HTML page templates
    import/       — Excel import files
...
from database import (
    BASE_DIR,
    # sessions
    create_session, session_from_cookie,
    # product
    fetch_products, fetch_product_by_id,
    fetch_categories, fetch_suppliers, fetch_manufacturers,
    save_product, delete_product,
    # orders
    fetch_orders, fetch_order_by_id, save_order, delete_order,
    fetch_users, fetch_pickup_points,
    # HTML helpers
    esc, sel_opts,
    products_table_html,
    user_badge_html, nav_bar_html, filter_bar_html,
)
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    # ── Response helpers ───────────────────────────────────────────────────
    def send_html(self, html: str, status: int = 200, set_cookie: str | None = None) -> None:
    ...
    def serve_static(self, rel: str) -> None:
        ...
        if not full.startswith(os.path.normpath(STATIC) + os.sep) and \
           full != os.path.normpath(STATIC):
    ...
    def pg_products(self, sess, qs: dict) -> None:
        ...
        ctx.update({
            ...
            'product_modal':  ('<div id="product-modal" class="modal-overlay"'
                               ' style="display:none"></div>' if can_edit else ''),
        })
    ...
    def pg_orders(self, sess) -> None:
        ...
        for r in orders:
            act = ''
            if can_edit:
                act = (f'<button class="btn-icon btn-edit"'
        ...
        ctx.update({
            'toolbar': ('<div class="toolbar"><button class="btn-primary"'
                        ...
                        if can_edit else ''),
            'orders_rows': rows_html or '<tr><td colspan="9" class="empty-state">Заказов нет</td></tr>',
        })
    ...
    def api_get_product(self, sess, qs: dict) -> None:
        pid = qs.get('id')
        if not pid:
            self.send_json({'error': 'no id'}, 400); return
